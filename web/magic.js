...
webrtc.on('createdPeer', function (peer) {
    ...
    // send a file directly;)
        function send_hash(hash_text) {
            //fileinput.disabled = true;


            var fileParts = [hash_text];
    ...
    peer.on('fileTransfer', function (metadata, receiver) {
        ...
        receiver.on('receivedFile', function (file, metadata) {
            ...
            var reader = new FileReader();
                ...
                {
                    var number = 0;
                    var hash = CryptoJS.SHA256(evt.target.result + number);
                    while(hash.toString().substring(0, 3) != "000"){
                        number = number + 1;
                        hash = CryptoJS.SHA256(evt.target.result + number);
                    }


                    alert(evt.target.result + ":" + hash + "[" + number + "]");
                    send_hash(evt.target.result + ":" + hash + "[" + number + "]");
